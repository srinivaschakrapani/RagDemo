// Non-streaming: waits for select_document + select_sections + fetch_content
// plus the downstream Gemma call to fully complete (cold starts can take ~60-90s).
export const maxDuration = 120;

export async function POST(request: Request) {
  const modalUrl = process.env.MODAL_RAG_PAGEINDEX_URL;
  const serveToken = process.env.MODAL_SERVE_TOKEN;
  if (!modalUrl || !serveToken) {
    return Response.json(
      { error: "server is missing MODAL_RAG_PAGEINDEX_URL/MODAL_SERVE_TOKEN" },
      { status: 500 }
    );
  }
